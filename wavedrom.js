...
//MZI-NOTE: Creates class from modules
var rec = require('./rec'),
    lane = require('./lane'),
    jsonmlParse = require('./create-element'),
...
function renderWaveForm (index, source, output) {
    ...
    //document.getElementsByClassName('wave-unit').onclick = alert("hi");
...
//MZI-NOTE: Reads source and creates wavelength
function renderWaveLane (root, content, index, lane) {
    ...
    for (j = 0; (j < content.length); j += 1 ) { //MZI_MOD: Show only couple signals at a time
        ...
        if (name) { // check name
            ...
            //lane.scale = lane.xs * (lane.hscale) * 2;
            ...
            if (content[j][1]) {
                //for (i = 0; i < content[j][1].length; i += 1) {
                    ...
                    b = document.createElementNS(w3.svg, 'use');
                     var my_id = name +'_use_' + i + '_' + j + '_' + index;
                     b.id = my_id;
					 if(clientdata.selected.id.indexOf(b.id) > -1){
						 content[j][1][i] = content[j][1][i] + "g";
					 }
                   ...
                   b.onmousedown = function(e) {
                       ...
                       //if(att_val.includes("pclk") || att_val.includes("nclk"))
						   //Copies the ID of the selected components and 
					   // pushes them so that the selection color moves along
					   // when the wave is moved.
                       if(att_val.includes("g")){ 
                            att_val = att_val.substr(0,att_val.length-1);
							var select_index = clientdata.selected.id.indexOf(e.target.id);
							if (select_index >= 0) {
							  clientdata.selected.id.splice( select_index, 1 );
							}
                       }else{
                           att_val += "g";
						   clientdata.selected.id.push(e.target.id);
                    ...
                    b.onmouseover=function(e){
                        ...
                        x.innerHTML = att_val+" "+e.target.id;
                        x.className = "show";
                ...
                if (content[j][2] && content[j][2].length) {
                    ...
                    if (labels.length !== 0) {
                        for (k in labels) {
                            var x_val = labels[k] * lane.xs + lane.xlabel - (i-tmp_i)*lane.xs;
                            //if (x_val > lane.xs*(window_size-start_time)) break; // Don't parse unnecessary text outside of window scope
                            if (content[j][2] && (typeof content[j][2][k] !== 'undefined')) {
                                title = tspan.parse(content[j][2][k]);
								var stl;
								stl = "font-size:"+(clientdata.zoom+9)+";";
								if (x_val > lane.xlabel){
                                ...
                                title.unshift(
                                    'text',
                                    {
                                        x: x_val, //MZI-MOD: Shiftablility of text
                                        y: lane.ym,
                                        'text-anchor': 'middle',
                                        'xml:space': 'preserve',
										style: stl
